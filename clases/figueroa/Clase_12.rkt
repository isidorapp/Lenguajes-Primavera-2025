...
(define (strict v)
  (match v
    ;; [(strict (interp expr env))] ;; sin cache
    
    [(exprV expr env cache)
     (if (not (unbox cache)) ;; si es #t, la expr no se ha reducido aún
         (let ([val (strict (interp expr env))]) ;; se reduce la expresión clausurada
           (set-box! cache val)  ;; se actualiza el cache con el valor obtenido
           val)
         (unbox cache))]
...
(define (interp expr env)
  (match expr
    ...
    [(if0 c t f)
     (if (num-zero? (strict (interp c env)))
         (interp t env)
         (interp f env))]
    ...
    [(app f e)
     ...
     (def (closureV the-arg the-body closed-env) (strict (interp f env))) ;; la evaluacion de f DEBE resultar en un closureV
     
     ;; (def new-env (extend-env the-arg (interp e env) closed-env)) ;; eager evaluation
     (def new-env (extend-env the-arg (exprV e env (box #f)) closed-env)) ;; lazy evaluation

     (interp the-body new-env) ;; sin cambios
     ]
    
    ))

(define (run prog)
  (let ([result (strict (interp (parse prog) empty-env))])
    (match result
      [(numV n) n]
      [(closureV arg body env) "<#procedure>"]
      [(exprV expr env cache) result])))
